...
class SceneManager:
    ...
    def _prepare_new_game(self, cast, script):
        ...
        self._add_score_first_player(cast, CENTER_X / 2, HUD_MARGIN)

        self._add_ball(cast)
        self._add_lives(cast)
        self._add_racket_first(cast, CENTER_X - RACKET_WIDTH / 2, SCREEN_HEIGHT - RACKET_HEIGHT, CENTER_X - RACKET_WIDTH / 2, 50 + RACKET_HEIGHT)
        #self._add_racket_first(cast, CENTER_X - RACKET_WIDTH / 2, 0 + RACKET_HEIGHT)
    ...
    def _add_score_first_player(self, cast, x, y):
        cast.clear_actors(SCORE_GROUP_FIRST)
        text = Text(SCORE_FORMAT, FONT_FILE, FONT_SMALL, ALIGN_CENTER)
        position = Point(x, y)
        label = Label(text, position)
        cast.add_actor(SCORE_GROUP_FIRST, label)
    ...
    def _add_racket_first(self, cast, X1, Y1, X2, Y2):
